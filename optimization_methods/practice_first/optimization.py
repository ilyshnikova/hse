...
class LineSearchTool(object):
	...
	def line_search(self, oracle, x_k, d_k, previous_alpha=None):
		...
		if self._method == 'Wolfe':
			...
			alpha = ls.scalar_search_wolfe2(phi, derphi=dphi, c1=self.c1, c2=self.c2)[0]
			if alpha:
				return alpha
			c = self.c1
		elif self._method == 'Constant':
...
def gradient_descent(oracle, x_0, tolerance=1e-5, max_iter=10000,
	...
	d_0n = tolerance * (oracle.grad(x_0).dot(oracle.grad(x_0)))

	while (np.linalg.norm(oracle.grad(x_k))**2 > d_0n):
